...
def make_move(player, board):
    ...
    new_move = None
    while True:
        print(f"{player_name(player)}'s move")
        move = input("Enter a number between 1 and 9")
        if move.isnumeric():
            move_int = int(move)
            if move_int <= 9 and move_int >= 1:
                for i in range(len(board)):
                    if i == move_int - 1:
                        new_move = i
            else:
            ...
            print("Not a number. Enter a number between 1 and 9")
            continue
        if board[new_move] == 0:
...
def check_win_horizontal(board):
    ...
    Iterates over every horizontal section of the board and determines whether a
    matching player name(not 0) is found.

    Parameters
    ----------
    board: list
        The board where a winner is being checked for
    '''
    if (board[0] != 0 and 
        board[0] == board[1] and 
        board[0] == board[2]):
...
def check_win_vertical(board):
    ...
    Iterates over every vertical section of the board and determines whether a
    matching player name(not 0) is found.
...
def check_win_diagonal(board):
    ...
    Iterates over every diagonal section of the board and determines whether a
    matching player name(not 0) is found.
